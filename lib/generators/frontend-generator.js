...
`;
  ...
  async generateDialogComponents(moduleName, options = {}) {
    try {
      ...
      const sharedFormTemplateName =
        this.templateVersion === 'v2' ? 'shared-form-v2.hbs' : 'shared-form.hbs';
      ...
      if (options.withImport) {
        ...
        console.log('🔍 Import Dialog Context:', JSON.stringify({
        ...
        const importTemplatePath = path.join(this.templatesDir, importDialogTemplate);

        if (fs.existsSync(importTemplatePath)) {
          const importTemplateContent = fs.readFileSync(importTemplatePath, 'utf8');
          const importTemplate = Handlebars.compile(importTemplateContent);

          try {
            const importCode = importTemplate(importContext);
            const importFile = path.join(outputDir, `${kebabName}-import.dialog.ts`);
            fs.writeFileSync(importFile, importCode);
            generatedFiles.push(importFile);
            console.log(`✅ Import dialog generated: ${importFile}`);
          } catch (templateError) {
            console.error('❌ Error compiling import dialog template:', templateError);
            console.error('Template context:', JSON.stringify(importContext, null, 2));
            throw templateError;
          }
        } else {
          console.warn(`⚠️ Import dialog template not found: ${importTemplatePath}`);
        }
      }
  ...
  async autoRegisterRoute(moduleName) {
    const appRoutesPath = path.join(this.projectRoot, 'apps/web/src/app/app.routes.ts');
    ...
    try {
      ...
      if (content.includes(`path: '${kebabName}'`)) {
        console.log(`⚠️ ${moduleName} route already registered in app.routes.ts`);
        return false;
      }
      ...
      content = content.slice(0, insertPosition) + routeEntry + content.slice(insertPosition);
